...
print("starts", len(row_starts), "extracted", len(extract_dual_column_appendix_section(lines, "Appendix 2. Amended recommendation wording", "MEDCMP_005")))
for position, start in enumerate(row_starts):
    ...
    elif not REASON_HINT.search(reason):
        print("REJECT", old_id, new_id, "no_hint")
    elif REASON_COLUMN_LEAK.search(reason):
        print("REJECT", old_id, new_id, "leak")
    elif NEW_SPAN_CONTAMINATION.search(new):
        print("REJECT", old_id, new_id, "contam")
