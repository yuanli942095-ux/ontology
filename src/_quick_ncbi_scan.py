...
def main() -> None:
    ...
    seen: set[str] = set()
    for term in terms:
        for book_id in esearch(term):
            nbk, title = resolve(book_id)
            if not nbk.startswith("NBK") or nbk in KNOWN or nbk in seen:
                continue
            seen.add(nbk)
            ...
            if found:
                ...
                print(f"HIT {nbk} {table_id} rows={rows} {title[:70]}")
                print(f"    {url}")
    print("DONE", len(seen), "books checked")
